...
#include "tinyphysics/geometry/ray2d.h"
#include "tinyphysics/geometry/algorithms.h"

using namespace tinyphysics;

TEST_CASE("Ray2D", "[geometry]")
{ 
    SECTION("Constructors")
    {
        //Constructor 2 Points
        Ray2D ray(Point2D(0., 0.), Vector2D(2., 3.));
        REQUIRE(almostEquals(ray.getDirectionVector().getX(), 2.));
        REQUIRE(almostEquals(ray.getDirectionVector().getY(), 3.));
    }
